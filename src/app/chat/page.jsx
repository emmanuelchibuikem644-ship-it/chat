...
export default function ChatPage() {
  ...
  useEffect(() => {
    if (!localStorage.getItem("access")) { router.replace("/login"); return; }
  ...
  return (
    <div style={{ height: "100dvh", display: "flex", flexDirection: "column", background: "var(--bg)", fontFamily: "var(--font-body)" }}>

      {/* ── Chat header ─────────────────────── */}
      <div style={{
        padding: "0 16px", height: "58px", borderBottom: "1px solid var(--border)",
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
          <Link href="/dashboard" style={{ display: "flex", alignItems: "center", gap: "8px", textDecoration: "none", color: "var(--text)" }}>
            <div style={{ width: "34px", height: "34px", borderRadius: "10px", background: "var(--sage)", display: "flex", alignItems: "center", justifyContent: "center", position: "relative" }}>
              <Heart size={15} color="white" fill="white" />
              <span style={{ position: "absolute", bottom: "-2px", right: "-2px", width: "10px", height: "10px", borderRadius: "50%", background: "#6bc78a", border: "2px solid var(--bg)" }} />
            </div>
            <div>
              <div style={{ fontSize: "14px", fontWeight: 600, lineHeight: 1.2 }}>Solace</div>
              <div style={{ fontSize: "11px", color: "var(--text-muted)" }}>Always here for you</div>
            </div>
          </Link>
        </div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "4px" }}>
          <button onClick={handleReset} title="Clear conversation" style={{
          ...
          <button onClick={handleLogout} title="Logout" style={{
            ...
            onMouseEnter={e => { e.currentTarget.style.background = "#fff0f0"; e.currentTarget.style.color = "#dc2626"; e.currentTarget.style.borderColor = "#fca5a5"; }}
      ...
      {/* ── Typing indicator ────────────────── */}
      {loading && (
        <div style={{ padding: "0 16px 12px", display: "flex", justifyContent: "flex-start" }}>
          <div style={{
